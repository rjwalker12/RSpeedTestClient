...
namespace NSpeedTest
{
    public class SpeedTestClient : ISpeedTestClient
    {
        ...
        private const int MaxUploadSize = 10; // 1000 KB
        ...
        public Settings GetSettings()
        {
            using (var client = new SpeedTestWebClient())
            {
                ...
                {
                    int latency = TestServerLatency(server);
        ...
        private static double TestSpeed<T>(IEnumerable<T> testData, Func<WebClient, T, Task<int>> doWork, int concurencyCount = 4)
        {
            var timer = new Stopwatch();
